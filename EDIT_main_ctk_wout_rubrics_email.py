...
failed_grading = {}
graded_images = {}
processed_data = {}  # Dictionary to store extracted text
image_buttons = {}  # Store image buttons by image path

MAX_COLUMNS = 5  # Limit to 5 squares per row
API_LIMIT = 5  # Maximum requests
PERIOD = 65  # Time window (seconds)

# API locks and timers for independent quota management
API_LOCK_1 = threading.Lock()
API_LOCK_2 = threading.Lock()
API_LOCK_3 = threading.Lock()
last_api_call_1 = deque(maxlen=API_LIMIT)  # Timer for extract/grade
last_api_call_2 = deque(maxlen=API_LIMIT)  # Timer for extract1/grade1
last_api_call_3 = deque(maxlen=API_LIMIT)  # Timer for extract1/grade1
api_sleep_status = [False, False, False]  # False = Active, True = Sleeping

stop_grading_1 = False  # Flag to stop thread 1
stop_grading_2 = False  # Flag to stop thread 2
stop_grading_3 = False  # Flag to stop thread 2
...
def send_email_with_attachment(to_email, subject, body, filename):
    sender_email = "group6thesisemail@gmail.com"  # Change to your email
    sender_password = "kjqd qdlt ownw viwn"  # Change to your email password
...
def save_session():
    ...
    filename = filedialog.asksaveasfilename(
        ...
        initialdir=sessions_directory,
        initialfile=f"session_{timestamp}.json"
    )
    if not filename:
        return
...
def load_session():
    ...
    for entry in saved:
        ...
        data = entry.get("data") or entry.get("ocr_text")  # fallback for old formats

        if not image_path or not data:
            continue

        # Check if file exists, fallback to archive if needed
        ...
        # Update session with verified image path
        entry["image_path"] = image_path
        processed_data[image_path] = data
        total_tasks += 1

        # Check completion status
        if "N/A" not in (data.get("name"), data.get("question"), data.get("answer")):
            completed_tasks += 1
        else:
    ...
    grading_progress_bar()
    update_progress_bar()
    messagebox.showinfo("Session Loaded", f"{len(saved)} entries restored from:\n{os.path.basename(filepath)}")
...
save_button = ctk.CTkButton(buttons_frame, text="Save Session", command=save_session,
                            font=font, fg_color=grey, hover_color=grey_hover, text_color=white, 
                            corner_radius=10, width=200, height=40)
save_button.pack(pady=(0,10))

load_button = ctk.CTkButton(buttons_frame, text="Load Session", command=load_session,
                            font=font, fg_color=grey, hover_color=grey_hover, text_color=white, 
                            corner_radius=10, width=200, height=40)
